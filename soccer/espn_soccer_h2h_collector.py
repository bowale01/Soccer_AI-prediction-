...
class ESPNSoccerH2HCollector:
    ...
    def get_today_fixtures(self) -> List[Dict]:
        ...
        print("🔍 Fetching today's fixtures from ESPN Soccer API...")
        ...
        for league_code, league_name in self.supported_leagues.items():
            try:
                ...
                if resp.status_code == 200:
                    ...
                    print(f"⚠️ {league_name}: HTTP {resp.status_code}")
                    
            except Exception as e:
                print(f"❌ {league_name}: {str(e)[:50]}")
    ...
    def get_team_h2h_data(self, team1_id: int, team2_id: int, league_code: str = "eng.1") -> Dict:
        ...
        try:
            ...
            # Fallback: Get recent matches for both teams and find common opponents
            print(f"🔄 Using fallback method for teams {team1_id} vs {team2_id}")
            return self._get_h2h_fallback(team1_id, team2_id, league_code)
            
        except Exception as e:
            print(f"⚠️ H2H collection failed: {str(e)[:50]}")
    ...
    def _get_h2h_fallback(self, team1_id: int, team2_id: int, league_code: str) -> Dict:
        ...
        try:
            ...
            if len(unique_h2h) >= 2:
                ...
                print(f"⚠️ Only {len(unique_h2h)} H2H matches found, using realistic fallback")
                return self._get_realistic_soccer_h2h_fallback(team1_id, team2_id)
        ...
        for match in matches:
            try:
                ...
                for competitor in competitors:
                    ...
                    if team_id == team1_id:
                        team1_score = int(score) if score else 0
                    elif team_id == team2_id:
                        team2_score = int(score) if score else 0
